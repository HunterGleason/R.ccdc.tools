...
Below we use *R.ccdc.tools::get_ccdc_ts* to use the [**CCDC**](https://doi.org/10.1016/j.rse.2014.01.011) coefficients to generate the synthetic surface reflectance time series at the pixel that intersects the coordinates above for the *green* and *nir* bands. Any XY coordinate that intersect the raster data can be specified, for any band exported in the CCDC raster. Notice that the time series has multiple breakpoints in both the green and nir bands since 1984, this point happens to be withing a heavily harvested location. 

```{r fig.width=6, fig.height=6}
#Name the CCDC bands as names are lost during export from Earth Engine, must provide the correct number of segments for the exported CCDC image 
ccdc_img<-name_ccdc_bands(rast(ccdc_img),8)
...
In the code below we apply the *R.ccdc.tools::gen_synthetic_img* for generating and plotting synthetic RGB Landsat scenes at several consecutive dates using the CCDC coefficients found in *ccdc_img*.Note that these images represent the predicted surface reflectance values and NOT the measured surface reflectance values, although they should agree closley.   

```{r fig.width=6, fig.height=6}
start_date<-date_to_jday("1995-01-01")
end_date<-date_to_jday("1996-01-01")
