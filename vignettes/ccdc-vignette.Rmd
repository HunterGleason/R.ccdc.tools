...
plot_ccdc_ts_bypoly(jul_15_2021,landcover,'landcover',730,'nir',60)

#Same plot as above, only showing quartiles
plot_ccdc_ts_quartiles(jul_15_2021,landcover,'landcover',730,'nir')
...
plot_ccdc_ts_bypoly(jul_15_2021,landcover,'landcover',730,'swir2',60)

#Plot CCDC 60 sample time series by polygon type (i.e., landcover) for the shortwave infrared band 2 ('swir2') over a 720 day period 
plot_ccdc_ts_quartiles(jul_15_2021,landcover,'landcover',730,'swir2')
```

## Comparing seasonal surface reflectance curves by landcover category (large areas of interest)

Often the size of the exported [**GEE-Script**](https://code.earthengine.google.com/054860fe5914f0851080b26da27b1120)) raster can be quite large, and many polygons may be used for exploring intra-annual surface reflectance between categories. The *R.ccdc.tools::sample_ccdc_by_catg* function achieves essentially the same result as *R.ccdc.tools::plot_ccdc_ts_quartiles* (but uses a sample like plot_ccdc_ts_bypoly), but uses [**SAGA-GIS**](https://saga-gis.sourceforge.io/en/index.html) with [**RSAGA**](https://cran.r-project.org/web/packages/RSAGA/index.html) for more efficient computation and demand on memory resources. It also returns the computed surface reflectance data back as a data.frame. Its use is shown in the code below.
